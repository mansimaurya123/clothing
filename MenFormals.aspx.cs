...
public partial class MenFormals : System.Web.UI.Page
{
    ...
    private void BindProductRepeater()
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("[procBindMenFormals]", con))
            {
    ...
    protected void txtFilterGrid1Record_TextChanged(object sender, EventArgs e)
    {
        ...
        con.Open();
        string qr = "SELECT A.*,B.*,C.Name, A.PPrice - A.PSellPrice AS DiscAmount,B.Name AS ImageName,  C.Name AS BrandName FROM tblProducts A INNER JOIN tblBrands C ON C.BrandID = A.PBrandID INNER JOIN tblCategory AS t2 ON t2.CatID = A.PCatID INNER JOIN tblSubCategory AS t3 ON t3.SubCatID = A.PSubCatID CROSS APPLY (SELECT TOP 1 *  FROM tblProductImages B WHERE B.PID = A.PID   ORDER BY B.PID DESC) B WHERE t2.CatName = 'Men' AND t3.SubCatName = 'Formals' AND A.PName like '" + txtFilterGrid1Record.Text + "%' order by A.PID desc ";
